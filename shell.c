...
int lsh_seer(char **args)
{
  ...
  //check for correct number of arguments
    if (args[1] == NULL || args[2] == NULL) {
        fprintf(stderr, "lsh: expected two arguments to \"grep\"\n");
        return 1;
    }

    const char *search_string = args[1];
    const char *filename = args[2];
  //open the file , read line by line, search for string, print lines that match
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        perror("lsh");
        return 1;
...
int lsh_execute(char **args)
{
  ...
  for (i = 0; i < lsh_num_builtins(); i++) {
    if (strcmp(args[0], builtin_str[i]) == 0) {
      return (*builtin_func[i])(args); /// loop through commands list and compare, input fromm used
    }
  }
